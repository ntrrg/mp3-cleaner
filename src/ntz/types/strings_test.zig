...
test "ntz.types.strings.concat" {
    ...
    const got = try strings.concat("hello, ", "world!", ally);
    defer ally.free(got);
...
test "ntz.types.strings.concat: null items" {
    ...
    const got = try strings.concat("hello, \x00", "\x00world!", ally);
    defer ally.free(got);
...
test "ntz.types.strings.concat: empty slice" {
    ...
    const got = try strings.concat("", "world", ally);
    defer ally.free(got);
...
test "ntz.types.strings.concat: empty items" {
    ...
    const got = try strings.concat("hello", "", ally);
    defer ally.free(got);
...
test "ntz.types.strings.concatMany" {
    ...
    const got = try strings.concatMany(&.{ "hello", ", ", "world", "!" }, ally);
    defer ally.free(got);
    try testing.expectEqlStrs(got, "hello, world!");
}
...
test "ntz.types.strings.concatMany: null items" {
    ...
    const got = try strings.concatMany(&.{ "hello, ", "\x00", "world!" }, ally);
    defer ally.free(got);
...
test "ntz.types.strings.concatMany: some empty strings" {
    ...
    const got = try strings.concatMany(&.{ "", "world" }, ally);
    defer ally.free(got);
    try testing.expectEqlStrs(got, "world");
}
...
test "ntz.types.strings.concatMany: empty edges" {
    ...
    const got = try strings.concatMany(&.{ "", "hello", "" }, ally);
    defer ally.free(got);
    try testing.expectEqlStrs(got, "hello");
}

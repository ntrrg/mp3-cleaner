...
test "ntz.types.slices.append: null item" {
    ...
    const got = try slices.append(u8, ally, "hello, world", 0);
    defer ally.free(got);
...
test "ntz.types.slices.concat: null items" {
    ...
    const got = try slices.concat(u8, ally, "hello, \x00", "\x00world!");
    defer ally.free(got);
...
test "ntz.types.slices.concatMany: null items" {
    ...
    const got = try slices.concatMany(u8, ally, &.{ "hello, ", "\x00", "world!" });
    defer ally.free(got);
